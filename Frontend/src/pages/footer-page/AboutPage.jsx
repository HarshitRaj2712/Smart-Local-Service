...
const AboutPage = () => {
  ...
  return (
    <div className="min-h-screen bg-(--bg-main) py-12 px-4 transition-colors duration-200">
      ...
      <div className="max-w-4xl mx-auto space-y-12 relative z-10">
        ...
        <button 
          onClick={() => navigate("/")}
          className="flex items-center gap-1.5 text-[11px] font-bold text-(--text-muted) hover:text-(--text-main) transition-colors"
        >
        ...
        >
          <span className="inline-block px-4 py-1 rounded-full bg-(--bg-surface) border border-(--border-color) text-[#007FFF] text-[10px] font-bold uppercase tracking-[0.2em]">
        ...
        <div className="text-center">
           <h2 className="text-xl font-bold text-(--text-main) mb-8">Meet the Creator</h2>
           <div className="relative inline-block group mb-6">
              <div className="w-32 h-32 md:w-36 md:h-36 rounded-full border-4 border-(--bg-main) shadow-2xl overflow-hidden mx-auto ring-8 ring-(--bg-muted)/30">
              ...
              <div className="absolute bottom-1 right-1 bg-(--bg-surface) p-2.5 rounded-full shadow-lg border border-(--border-color) text-[#007FFF]">
                <User size={18} fill="currentColor" />
              </div>
           </div>
           <h3 className="text-xl font-bold text-(--text-main) tracking-tight">Harshit</h3>
           <p className="text-[10px] font-bold text-[#007FFF] uppercase tracking-[0.2em] mt-1 mb-6">Full Stack Developer & Creator</p>
           
           <div className="bg-(--bg-panel) backdrop-blur-xl border border-(--border-color) p-6 rounded-[28px] shadow-sm max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {stats.map((s, i) => (
            <MotionDiv
              ...
              className="bg-(--bg-panel) backdrop-blur-md border border-(--border-color) p-6 rounded-3xl shadow-sm text-center hover:shadow-md transition-shadow"
            >
              <div className="bg-[#007FFF]/5 w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-4 italic">
                {s.icon}
              </div>
              <h4 className="text-lg font-bold text-(--text-main)">{s.label}</h4>
              <p className="text-[10px] font-bold text-(--text-muted) uppercase tracking-tight">{s.sub}</p>
            </MotionDiv>
          ))}
        </div>
        ...
        <div className="pt-10">
          <div className="text-center mb-12 md:mb-14">
             <span className="bg-(--bg-surface) backdrop-blur-sm px-4 py-1 rounded-full text-[10px] font-bold text-[#007FFF] border border-(--border-color) shadow-sm uppercase tracking-[0.2em]">Journey</span>
             <h2 className="text-3xl md:text-4xl font-bold text-(--text-main) mt-4 tracking-tight">The Evolution of the Idea</h2>
             <p className="text-sm text-(--text-muted) mt-3 max-w-xl mx-auto">From a frustrating daily problem to a trusted local-services platform, one milestone at a time.</p>
          </div>
            ...
            <div className="space-y-5 md:space-y-7">
              {journeySteps.map((step, idx) => {
                ...
                return (
                  ...
                  >
                    <div className={`absolute left-0 md:left-1/2 md:-translate-x-1/2 top-7 w-6 h-6 rounded-full border-4 border-(--bg-main) shadow ${step.color} z-10`}></div>

                    <div
                      className={`group rounded-[28px] border border-(--border-color) bg-(--bg-surface) backdrop-blur-sm p-6 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5 ${
                    ...
                    >
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div>
                          <h4 className="text-sm md:text-base font-bold text-(--text-main) tracking-tight">{step.title}</h4>
        ...
        {/* --- TECH STACK --- */}
        <div className="relative overflow-hidden bg-(--bg-panel) backdrop-blur-md border border-(--border-color) p-6 md:p-8 rounded-[30px] shadow-sm">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between gap-4 mb-7">
              <h2 className="text-xs font-bold text-(--text-muted) uppercase tracking-[0.3em]">Modern Technology Stack</h2>
            ...
            <div className="grid md:grid-cols-[1.15fr_0.85fr] gap-5 items-start">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="text-[9px] font-bold text-(--text-muted) uppercase tracking-wider">{tech.role}</span>
                      </div>
                      <h3 className="text-sm font-bold text-(--text-main) group-hover:text-[#007FFF] transition-colors">{tech.name}</h3>
                    </MotionDiv>
                  );
                })}
              </div>
              ...
              <MotionDiv
                ...
                className="rounded-3xl border border-[#007FFF]/20 bg-linear-to-br from-(--bg-surface) to-[#007FFF]/5 p-6 shadow-md"
              >
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <h3 className="text-xl font-bold text-(--text-main) tracking-tight">{activeTech.name}</h3>
                  </div>
                </div>
                <p className="text-sm text-(--text-muted) leading-relaxed font-medium">{activeTech.description}</p>
...
const SocialBtn = ({ icon, label, href }) => (
  <a
    ...
    className="flex items-center gap-2 px-4 py-2 bg-(--bg-surface) rounded-xl border border-(--border-color) text-[10px] font-bold text-(--text-muted) hover:text-[#007FFF] hover:bg-(--bg-muted) hover:shadow-md transition-all uppercase tracking-widest"
  >
    {icon} {label}
  </a>
);
